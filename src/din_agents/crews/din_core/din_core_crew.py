...
class DinCoreCrew:
    ...
    def assess_patch_contract(self) -> Task:
        return Task(
            config=self.tasks_config["assess_patch_contract"],  # type: ignore[index]
        )

    @task
    def review_registry_and_runtime(self) -> Task:
        return Task(
            config=self.tasks_config["review_registry_and_runtime"],  # type: ignore[index]
        )

    @task
    def plan_rust_validation(self) -> Task:
        return Task(
            config=self.tasks_config["plan_rust_validation"],  # type: ignore[index]
        )
